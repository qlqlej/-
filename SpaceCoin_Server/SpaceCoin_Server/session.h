...
public:
	...
	int				missile_up = 0, slash_up = 0, slash_size_up = 0, ultimate_up = 0;
	float			hp;
	...
	void InitLogin(SC_LOGIN_INFO_PACKET& info_packet, QUEST_DATA& quest_data, CS_LOGIN_PACKET* login_packet);
	void InitTestLogin(SC_LOGIN_INFO_PACKET& info_packet, QUEST_DATA& quest_data, CS_TEST_LOGIN_PACKET* login_packet);
	...
	void SendToQuestData();
	void SendToAddPlayer(SESSION& target);
	void SendToAddMonster(MONSETR& target);
	void SendToMovePlayer(SESSION& target);
	void SendToTestMovePlayer(SESSION& target, unsigned client_time);
	void SendToChangeCharacter(SESSION& target);
	void SendToChangeWeapon(SESSION& target);
	void SendToAttackSkill(CS_ATTACK_SKILL_PACKET* packet, SESSION& target);
	void SendToSpawnShip(CS_SPAWN_SPACESHIP_PACKET* packet, SESSION& target);
