...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_LOGIN_INFO:
	{
		...
		int my_id = ci;
		client_map[login_packet->sessionID] = my_id;
		g_clients[my_id].id = login_packet->sessionID;
	...
	case SC_MOVE_PLAYER: 
	{
		...
		if (move_packet->sessionID < MAX_CLIENTS) {
			int my_id = client_map[move_packet->sessionID];
			if (-1 != my_id) {
	...
	case SC_MONSTER_INFO: break;
	case SC_MOVE_MONSTER: break;
	case SC_CHANGE_MAP: break;
	case SC_CHARACTER_CHANGE: break;
	case SC_WEAPON_CHANGE: break;
	case SC_ATTACK_SKILL: break;
	case SC_SPAWN_SPACESHIP: break;
	case SC_REMOVE_MONSTER: break;
	case SC_DAMAGE_MONSTER: break;
	case SC_ATTACK_MONSTER: break;
	case SC_GET_ITEM: break;
	case SC_ITEM_BUY: break;
	case SC_BUY_SELL: break;
	case SC_COIN_PRICE: break;
	case SC_MONSTER_ATTACK: break;
	case SC_QUEST: break;
	case ALL_STATE: break;
...
void Adjust_Number_Of_Client()
{
	...
	CS_TEST_LOGIN_PACKET l_packet;

	int temp = num_connections;
	sprintf_s(l_packet.name, "%d", temp);
	l_packet.size = sizeof(l_packet);
	l_packet.type = CS_TEST_LOGIN;
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			CS_TEST_MOVE_PACKET my_packet;
			my_packet.size = sizeof(my_packet);
			my_packet.type = CS_TEST_MOVE;
			my_packet.sessionID = g_clients[i].id;
			switch (rand() % 4) {
