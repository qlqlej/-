#include "trigger_ice_to_space.h"
#include <chrono>
...
AMainGamePlayerController* pcis;

Atrigger_ice_to_space::Atrigger_ice_to_space()
{
	...
	volume_box->OnComponentBeginOverlap.AddDynamic(this, &Atrigger_ice_to_space::OnOverlapBegin);
}

void Atrigger_ice_to_space::BeginPlay()
{
	Super::BeginPlay();

	pcis = Cast<AMainGamePlayerController>(UGameplayStatics::GetPlayerController(GetWorld(), 0));
}

void Atrigger_ice_to_space::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void Atrigger_ice_to_space::OnOverlapBegin(class UPrimitiveComponent* OverlappedComp, class AActor* OtherActor, class UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor && (OtherActor != this) && OtherComp)
	{
		if (OtherActor->GetClass()->IsChildOf(Aspace_ship::StaticClass())) {
			...
			GameInstanceRef->map_name = "IcePlanet";
