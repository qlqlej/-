...
def requestCoinAPI():  # 코인 종목 및 정보
    ...
    for key in data:
        coin_info = []
        coin_info.append(key)
        info = data[key]
        coin_info.append(info["opening_price"])  # 시가 00시 기준
        coin_info.append(info["closing_price"])  # 종가 00시 기준
        coin_info.append(info["min_price"])  # 저가 00시 기준
        coin_info.append(info["max_price"])  # 고가 00시 기준
        coin_info.append(info["units_traded"])  # 거래량 00시 기준
        coin_info.append(info["prev_closing_price"])  # 거래금액 00시 기준
        coin_info.append(info["acc_trade_value"])  # 전일종가
        coin_info.append(info["units_traded_24H"])  # 최근 24시간 거래량
        coin_info.append(info["acc_trade_value_24H"])  # 최근 24시간 거래금액
        coin_info.append(info["fluctate_24H"])  # 최근 24시간 변동가
        coin_info.append(info["fluctate_rate_24H"])  # 최근 24시간 변동률
        coin_items.append(coin_info)
...
def requestInfo(stock_symbol, field):  # 코인 정보
    api_url = "https://api.bithumb.com/public/ticker/ALL_KRW"
    req = request.Request(api_url)
    res = request.urlopen(req)
    resultString = res.read().decode('utf-8')
